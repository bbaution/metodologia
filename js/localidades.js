...
async function guardar() {
    try {
        ...
        if (!inputValido) {
            const resp = await axios.post("http://localhost:3000/localidades", {
                direccion: direccion.value
            });
            alert("la localidad " + direccion.value + " se guardó con éxito!");
        }
    } catch (error) {
        console.log(error);
        alert("Error: " + error.message);
...
async function mostrar(id) {
    try {
        ...
        btnAgr.hidden = true;
        auxiliar = id;
        resp = await axios.get("http://localhost:3000/localidades/" + id)
...
async function actualizar() {
    try {
        resp = await axios.put("http://localhost:3000/localidades/" + auxiliar, {
...
async function borrar(id) {
    try {
        if (await deudaAlumno(id)) {
            ...
            await axios.delete("http://localhost:3000/localidades/" + id);
            listar()
        }
    } catch (error) {
        console.log(error)
...
async function listar() {
    try {
        resp = await axios.get("http://localhost:3000/localidades")
        listado.innerHTML = "";
        resp.data.forEach(element => {
            listado.innerHTML += '<hr> <li class="listado">' + '<button onclick="borrar(' + element.id + ')" class="bg-danger me-1"><i class="fa-solid fa-trash"></i></button>' + '<button onclick="mostrar(' + element.id + ')"class="bg-success me-2"><i class="fa-solid fa-pencil"></i></button>' + ' Id: ' + element.id + ' |' +' Nombre: '+element.direccion +"</li>"
        })
    } catch (error) {
        console.log(error)
        alert("error")
