...
async function guardar() {
    try {
        ...
        if (!inputValido) {
            const resp = await axios.post("http://localhost:3000/canal", {
                direccion: direccion.value
            });
...
async function mostrar(id) {
    try {
        ...
        btnAgr.hidden = true;
        auxiliar = id;
        resp = await axios.get("http://localhost:3000/canal/" + id)
...
async function actualizar() {
    try {
        resp = await axios.put("http://localhost:3000/canal/" + auxiliar, {
...
async function borrar(id) {
    try {
        if (await deudaAlumno(id)) {
            ...
            await axios.delete("http://localhost:3000/canal/" + id);
            listar()
        }
    } catch (error) {
        console.log(error)
...
async function listar() {
    try {
        resp = await axios.get("http://localhost:3000/canal")
        listado.innerHTML = "";
        resp.data.forEach(element => {
            listado.innerHTML += '<hr> <li class="listado">' + '<button onclick="borrar(' + element.id + ')" class="bg-danger me-1"><i class="fa-solid fa-trash"></i></button>' + '<button onclick="mostrar(' + element.id + ')"class="bg-success me-2"><i class="fa-solid fa-pencil"></i></button>' + ' Id: ' + element.id + ' |' +' Medio: '+element.direccion +"</li>"
        })
    } catch (error) {
        console.log(error)
        alert("error")
